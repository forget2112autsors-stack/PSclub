...
export function Settings() {
  ...
  return (
    <div className="max-w-4xl space-y-10">
      ...
      <section>
        <h2 className="mb-3 text-sm font-medium text-slate-300">Tariflar</h2>
        <ul className="space-y-2">
          {tariffs.data?.map((tariff) => (
            <li key={tariff.id} className="flex items-center justify-between rounded-lg bg-slate-900 px-4 py-3 text-sm">
          ...
          {tariffs.data?.length === 0 && (
            <li className="text-sm text-slate-500">
              Hali kiritilmagan — TZ 12-bo'limidagi 1-savolga javob kelgach to'ldiriladi.
            </li>
          )}
        </ul>
      </section>
